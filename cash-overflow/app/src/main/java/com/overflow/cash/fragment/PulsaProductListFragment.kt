...
class PulsaProductListFragment : BaseFragment(), LoadPulsaProductContract.View ,SendOrderPulsaContract.View{
    ...
    private fun doOrder(){
        activity!!.showMessage(getString(R.string.top_up), getString(R.string.are_you_sure_topup).replace("{0}", productCode), object:MessageButtonHandle(){
    ...
    override fun onOrderSended(result: Data) {
        ...
        showProgress(false)
        activity!!.showMessage(getString(R.string.top_up), getString(R.string.transaction_in_progress).replace("{0}", productCode), object :MessageButtonHandle(){
